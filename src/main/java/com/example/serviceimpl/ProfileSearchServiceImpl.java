...
public class ProfileSearchServiceImpl implements ProfileSearchService {
    ...
    public PageResponse<ProfileSearchResultDTO> searchProfiles(ProfileSearchRequestDTO request) {
        ...
        if (request != null) {
            ...
            if (request.getAgeFrom() != null
                    && (request.getAgeFrom() < 18 || request.getAgeFrom() > 100)) {
            ...
            if (request.getAgeTo() != null
                    && (request.getAgeTo() < 18 || request.getAgeTo() > 100)) {
        ...
        int page = request != null
                && request.getPage() != null
                && request.getPage() >= 0
                ? request.getPage()
                : 0;

        int size = request != null
        ...
        Sort sort = buildSort(
                request != null ? request.getSortBy() : "relevance",
                request != null ? request.getSortOrder() : "desc"
        );
         ...
         * IMPORTANT:
         *
         * Employment Status is already part of
         * ProfileSearchRequestDTO as employmentStatusId.
         *
         * The complete request is passed to
         * ProfileSearchSpecification where all search filters
         * are applied.
         */
        Page<Profile> profilePage = profileRepository.findAll(
                ProfileSearchSpecification.buildSearchSpecification(
                        request,
                        currentUserId
                ),
                pageable
        ...
        java.util.Map<Long, String> userPhotoMap =
                new java.util.HashMap<>();

        if (!userIdsNeedingPhotos.isEmpty()) {

            List<UserPhoto> primaryPhotos =
                    userPhotoRepository.findByUserIdInAndPrimaryPhotoTrue(
                            userIdsNeedingPhotos
                    );
    ...
    private Sort buildSort(
        ...
        return switch (sortBy.toLowerCase()) {
            ...
            case "age_low_high", "age_asc" ->
                    Sort.by("dateOfBirth").descending();

            case "age_high_low", "age_desc" ->
    ...
    private ProfileSearchResultDTO mapToSearchResultDTO(
            Profile p,
            java.util.Map<Long, String> userPhotoMap
    ) {
        ...
        return ProfileSearchResultDTO.builder()
                ...
                .drinkingName(
                        p.getDrinking() != null
                                ? p.getDrinking().getName()
                                : null
                )

                .manglikStatusId(
                        p.getManglikStatus() != null
